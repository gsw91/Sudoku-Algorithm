    ...
    public class BoardTestSuite {
        ...
        public void testAddTaskListAverageWorkingOnTask(){
            ...
            List<Long> days = project.getTaskLists().stream()
                    ...
                    .map(a -> LocalDate.now().toEpochDay() - a.getCreated().toEpochDay())
                    .collect(toList());

            long daysSum = days.stream().mapToLong(Long::longValue).sum();

            double averageActual = (double) daysSum / (double) tasks.size();
            double averageExpected = (10.0);

            //Then
            Assert.assertEquals(3, tasks.size());
            Assert.assertEquals(averageExpected, averageActual,0.01);
        }
